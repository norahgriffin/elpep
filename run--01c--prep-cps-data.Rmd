...
cps_sub <- 
  ...
  .[CPSID != 0 &                         # This rules out ASEC over-sample households (see documentation on `CPSID`)
    #(ASECFLAG == 1 | is.na(ASECFLAG)) & # This includes both ASEC and basic monthly (where ASECFLAG is NA)
    (!STATEFIP %in% c(2, 15))          & # This excludes HI and AK
     between(RELATE, 101, 1001)] %>%    # This subsets to only related members of the family              
  select(-COUNTY, -OCC, -CLASSWKR, -FAMID) # dropping variables not currently used.
...
cps_timing[j = table(months_since_base)]

cps_timing[j = `:=`(is_baseline = months_since_base == 0,
                    # /!\ This selects a single post-period survey. Notably, it's the most recent in which
...
cps_timing_sub <- 
  cps_timing[is_baseline == TRUE | is_post == TRUE]
...
<!-- These codes are consistent with being the householder themselves (RELATE == 101), which is by far the the most common, or otherwise the child, parent, grandchild, or "other" relative. In short, because this implies that the head of household has no spouse present for whom FAMINC would ostensibly represent a combined income, we treat them as would-be single filers. -->
...
cps_family <- 
  ...
  .[j = `:=`(fam_size = .N,
             ...
             n_kids = sum(1*RELATE %in% c(301, 303)), # 301 - child of head of household, 303 is step-child
             n_kids_le16 = sum(AGE <= 16),
             ...
             single_filer_flag = RELATE == 101       & MARST %in% 3:6,  # this is HOH is single, separated, divorced or widowed
              joint_filer_flag = RELATE %in% 201:203 & EMPSTAT == 10,   # assumed if spouse (opposite or same sex) is employed. /!\ There's potential for greater detail.
                hoh_filer_flag = RELATE == 101       & MARST %in% 1:2), # this is HOH who is married with spouse either present or absent, but where spouse does not work
...
* Reduction of amounts with full phase-out at $80,000 for single filers, $120,000 for head of household, and $160,000 for married joint filers. This implied a significantly higher rate of phase-out, and one that is variable
...
cps_pov <-
  ...
  .[j = `:=`(eip1_est = 
               case_when(filing_status == "single" ~ pmax(0, 1200 + n_own_kids_le16*500 - 0.05*(faminc_draw0 -  75000)),
                         filing_status == "hoh"    ~ pmax(0, 2400 + n_own_kids_le16*500 - 0.05*(faminc_draw0 - 112500)),
                         filing_status == "joint"  ~ pmax(0, 2400 + n_own_kids_le16*500 - 0.05*(faminc_draw0 - 150000))),
             eip2_est = 
               case_when(filing_status == "single" ~ pmax(0,  600 + n_own_kids_le16*600 - 0.05*(faminc_draw0 -  75000)),
                         filing_status == "hoh"    ~ pmax(0, 1200 + n_own_kids_le16*600 - 0.05*(faminc_draw0 - 112500)),
                         filing_status == "joint"  ~ pmax(0, 1200 + n_own_kids_le16*600 - 0.05*(faminc_draw0 - 150000))),
             # /!\ As a temporary assumption, we simply assume that the full payment is received if under the
             # cutoff, as the interval of benefit decline is so narrow
             eip3_est = 
               case_when(filing_status == "single" ~ ifelse(faminc_draw0 <=  80000, 1400*fam_size, 0),
                         filing_status == "hoh"    ~ ifelse(faminc_draw0 <= 120000, 1400*fam_size, 0),
                         filing_status == "joint"  ~ ifelse(faminc_draw0 <= 160000, 1400*fam_size, 0)),
...
cps_pov_monthly <- 
  ...
  select(-matches("INCTOT|INCWAGE|INCUNEMP|POVERTY|ASECFLAG"))
```
  ...
  .[j = `:=`(hoh_industry_vulnerable = ifelse(hoh_industry == "NA", 
             ...
             fhoh_empl = factor(hoh_empl,
                                ...
                                labels = c("HOH Not Employed",
                                           "HOH Employed")))] %>% 
  select(CPSID, months_since_base, hoh_gender, hoh_educ, hoh_race, hoh_hispanic, hoh_industry, hoh_industry_vulnerable, hoh_empl)

cps_base_hoh <- cps_hoh[months_since_base ==  0] %>% select(-months_since_base)
cps_post_hoh <- cps_hoh[months_since_base == 15] %>% select(-months_since_base)
```
...
cps_hoh_pre_post <- 
  merge(cps_elig[RELATE == 101 & faminc_draw < 150000 & months_since_base == 0] %>% unique(), # & YEAR_MONTH == "2019-06"],
        ...
        #all = TRUE,
        suffixes = c("_b", "_p")) %>% 
  select(-months_since_base_b, -months_since_base_p) %>% 
