...
pop_age_groups_2020 <-
  ...
  mutate(est_2020_02   = round(totallt18_2020*per02_acs,   0),
         ...
         moe_2020_612  = moe_product(totallt18_2020, per5_acs,    0, per612_moe),
...
pop_bg_2010 <- 
  ...
  merge(age_mapping_bg %>% select(variable = name, big_agegroup),
        by = "variable") %>% 
  group_by(GEOID, big_agegroup) %>% 
  summarize(origpop2010 = sum(value)) %>% 
...
```{r examine time path of pct 0-2 out of under 5}
pcts_under5 <- 
  ests_with_forecast %>% 
...
```{r examine breakdown of ages based on size of the under 5 population}
