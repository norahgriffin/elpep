...
acs1_child1317_baseyr <-
  acs1_child[YEAR == base_year & 
               child_agegroup %in% c("13to14", "15to17")]
```
...
incpov_ref_value  <- "0%-80%"  # "0%-100%"
#incpov_ref_value  <- "0%-74%"  # "0%-100%
#incpov_ranges_for_estimation <- c("0to74", "75to149", "150to199", "200to299")
...
sae_controls_simple_1317 <- 
  paste0("incpov_12to17_r", incpov_ranges_for_estimation, "_est")
...
sae_controls_added_2534_1317 <- 
  c(sae_controls_simple_1317, sae_extra_controls)
```
...
pop_by_age_base <- 
  ...
  select(GEOID, 
         ...
         age_13to17_count_base = age_13to17_count)
...
run_sae_combos_bycontrols <- function(vul_vars, sae_input_data, my_controls) {
  ...
  else if (identical(sae_input_data, acs1_child1317_sae)) {
    pop_n_var    <- "age_13to17_count_base"    # "age_6to12_count" would be the alternative, from ACS5 data
    pop_n_se_var <- "age_13to17_count_se_base" # "age_6to12_se"    would be the alternative, from ACS5 data
  }
  #browser() 
  ...
  if (parallelize_sae) {
    ...
    runlist <- 
      list(sae_results_incpov_simple_05     = list(incpov_spec_field,      acs1_child05_sae,  sae_controls_simple_05),
           ...
           sae_results_incpov_simple_1317    = list(incpov_spec_field,      acs1_child1317_sae, sae_controls_simple_1317),
           ...
           sae_results_incpov_ctrls2534_1317 = list(incpov_spec_field,      acs1_child1317_sae, sae_controls_added_2534_1317),
    ...
    sae_results_incpov_simple_1317    <- run_sae_combos_bycontrols(incpov_spec_field,      acs1_child1317_sae, sae_controls_simple_1317)
    ...
    sae_results_incpov_ctrls2534_1317 <- run_sae_combos_bycontrols(incpov_spec_field,      acs1_child1317_sae, sae_controls_added_2534_1317)
    ...
    sae_results_ccdf_wk_pov_612       <- run_sae_combos_bycontrols(work_incpov_spec_field, acs1_child612_sae, sae_controls_added_2534_612)
  }
...
#sae_results_incpov_simple_1317_out %>% 
...
#sae_results_incpov_simple_1317[["comp_viz"]] +
...
cbind(sae_results_incpov_simple_05[["agg_comp_fh"]]$agg_comp_cor     %>% mutate(simple_05     = round(V1, 3)) %>% select(-V1),
      ...
      sae_results_incpov_simple_1317[["agg_comp_fh"]]$agg_comp_cor    %>% mutate(simple_1317    = round(V1, 3)) %>% select(simple_1317),
      ...
      sae_results_incpov_ctrls2534_1317[["agg_comp_fh"]]$agg_comp_cor %>% mutate(ctrls2534_1317 = round(V1, 3)) %>% select(ctrls2534_1317)) %>% 
...
cbind(sae_results_incpov_simple_05[["agg_comp_model"]]$agg_comp_cor     %>% mutate(simple_05     = round(V1, 3)) %>% select(-V1),
      ...
      sae_results_incpov_simple_1317[["agg_comp_model"]]$agg_comp_cor    %>% mutate(simple_1317    = round(V1, 3)) %>% select(simple_1317),
      ...
      sae_results_incpov_ctrls2534_1317[["agg_comp_model"]]$agg_comp_cor %>% mutate(ctrls2534_1317 = round(V1, 3)) %>% select(ctrls2534_1317)) %>% 
...
pov_p <- 
  lapply(c("05", "612", "1317"),
         function(a) {
           get(glue("sae_results_incpov_ctrls2534_{a}"))$sae_out %>% 
             #Norah changed this from 0-50 and 50-100??? Not sure about it but i did it
             filter(vc_value %in% c("0%-80%", "80%-100%")) %>% 
             group_by(GEOID) %>% 
...
tractShp_pov <- 
  ...
  left_join(pov_p[[2]] %>% rename(acs1_pov_612      = acs1_pov,
            ...
            by = "GEOID") %>% 
  left_join(pov_p[[2]] %>% rename(acs1_pov_1317      = acs1_pov,
                                  sae_pov_fh_1317    = sae_pov_fh,
                                  sae_pov_model_1317 = sae_pov_model),
            by = "GEOID") %>% 
...
map_sae_components <- function(dt, fill_field, field_label, val_limits, save_label) {
  ...
  pumaShp_sub <-
    pumaShp %>% 
    filter(PUMACE10 %in% pumas_to_keep)
...
limits_612 <- range(with(tractShp_pov, c(acs1_pov_612)), # sae_pov_fh_612, sae_pov_model_612
                    na.rm = TRUE)
limits_1317 <- range(with(tractShp_pov, c(acs1_pov_1317)), # sae_pov_fh_612, sae_pov_model_612
                    na.rm = TRUE)
```
...
map_sae_components(my_dt, "acs1_pov_612",      "", val_limits = limits_612, save_label = "Poverty (ACS1), age 6-12")
map_sae_components(my_dt, "acs5_pov",         "", val_limits = limits_612, save_label = "Poverty (ACS5), age 6-12")
map_sae_components(my_dt, "sae_pov_model_612", "", val_limits = limits_612, save_label = "Poverty (SAE), age 6-12")

map_sae_components(my_dt, "acs1_pov_1317",      "", val_limits = limits_1317, save_label = "Poverty (ACS1), age 13-17")
map_sae_components(my_dt, "acs5_pov",         "", val_limits = limits_1317, save_label = "Poverty (ACS5), age 13-17")
map_sae_components(my_dt, "sae_pov_model_1317", "", val_limits = limits_1317, save_label = "Poverty (SAE), age 13-17")
