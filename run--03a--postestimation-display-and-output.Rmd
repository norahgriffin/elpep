...
nowcast_parallel_deep_pov <- 
  ...
  mutate(base_share = share_0_80)

nowcast_parallel_hs_elig <- 
  nowcast_parallel_setup %>% 
  filter(outcome_var == "hs_elig_post_adj",
         str_detect(sae_spec, "incpov_ctrls2534")) %>% 
  mutate(base_share = share_0_80 + share_80_100)

nowcast_parallel_pfa_elig <- 
  nowcast_parallel_setup %>% 
  filter(outcome_var == "pfa_elig_post_adj",
         str_detect(sae_spec, "incpov_ctrls2534")) %>% 
  mutate(base_share = share_0_80 + share_80_100 + share_100_200)
...
nowcast_parallel_calworks_elig <- 
  nowcast_parallel_setup %>% 
  filter(outcome_var == "calworks_elig_inc_adj",
         str_detect(sae_spec, "incpov_ctrls2534")) %>%
  mutate(base_share = share_0_80)
...
pop_by_age_long$GEOID = str_pad(pop_by_age_long$GEOID, width = 11, pad = "0")
...
```
```{r}
plot_elig_diff_vs_char(outcome_string = "calworks_elig",
                       outcome_desc   = "CalWORKS Eligibility", 
                       char_field     = "raceeth_wh_nonh_est",
                       char_desc      = "% White Non-Hisp in Tract",
                       save_tag       = "by white residence")
```
